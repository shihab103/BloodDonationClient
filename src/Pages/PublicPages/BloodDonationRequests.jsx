...
// ✅ NEW: 90 days eligibility check
const checkDonationEligibility = (requests, userEmail) => {
  const doneDonations = requests.filter(
    (r) => r.donorEmail === userEmail && r.donationStatus === "done",
  );

  if (doneDonations.length === 0) return { eligible: true };

  const latest = doneDonations.sort(
        ...
        ${
          isInProgress && isDonor
            ...
            : isInProgress
              ...
              : "bg-red-600 text-white hover:bg-gray-900"
        }`}
    >
      <AnimatePresence>
        {isAnimating && (
...
const BloodDonationRequests = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const extractedUpazilas =
          upazilaData.find((item) => item.name === "upazilas")?.data || [];
  ...
  const fetchRequests = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}/public-donation-requests`,
      );
  ...
  const handleDonate = async (id) => {
    const eligibility = checkDonationEligibility(requests, user?.email);

    if (!eligibility.eligible) {
      Swal.fire({
        icon: "warning",
        title: "Not Eligible Yet",
        text: `You can donate again after ${eligibility.remainingDays} days.`,
  ...
  const handleCancel = async (id) => {
    try {
      await axios.patch(
        `${import.meta.env.VITE_API_URL}/cancel-donation/${id}`,
      );
      fetchRequests();
    } catch (error) {
  ...
  const filteredRequests = requests.filter((req) => {
    ...
    if (filterDistance !== "All" && dbUser && upazilas.length > 0) {
      const userUpazilaGeo = upazilas.find(
        (u) => u.name === dbUser.upazilaName,
      );
      const reqUpazilaGeo = upazilas.find(
        (u) => u.name === req.recipientUpazila,
      );
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
              {filteredRequests.map((req) => {
                ...
                return (
                  <div
                    key={req._id}
                    className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 flex flex-col justify-between hover:shadow-2xl transition-all duration-500 relative overflow-hidden group"
                  >
                    ...
                    <div>
                      <div className="mb-6">
                        <h3 className="text-2xl font-black text-gray-800 uppercase leading-none">
                          {req.recipientName}
                        </h3>
                        {distance !== null && (
                        ...
                        <span
                          className={`inline-block mt-3 px-4 py-1 rounded-full text-[10px] font-black tracking-widest uppercase ${
